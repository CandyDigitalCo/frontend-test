...
const searchGetters = {
  searchResults: state => {
    state.searchItems = state.searchItems.filter(elem => elem.searchableSlug.indexOf(state.searchSlug.toLowerCase()) !== -1 || elem.category.indexOf(state.searchSlug.toLowerCase()) !== -1).sort((a, b) => {
